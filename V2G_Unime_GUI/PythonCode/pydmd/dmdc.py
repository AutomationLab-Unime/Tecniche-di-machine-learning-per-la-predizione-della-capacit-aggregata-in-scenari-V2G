...
class DMDc(DMDBase):
    ...
    "The original reconstructed_data"
    def reconstructed_data_(self, control_input=None):
        """
    ...
    def reconstructed_data(self, open_loop, X, control_input=None, cut_negative_AAC=False):
        """
        Return the reconstructed data, computed using the `control_input`
        argument. If the `control_input` is not passed, the original input (in
        the `fit` method) is used. The input dimension has to be consistent
        ...
        #open loop reconstruction 
            ...
            #controllo la dimensione solo se uso snapshot, altrimenti posso passare qualsiasi dimensione di dati
            if arr.shape != expected_shape:
                raise ValueError(
                    f"Invalid shape: expected {expected_shape}, got {arr.shape}"
                )
            # taglia i valori negativi a zero, per motivi reali la AAC non può essere negativa
            if cut_negative_AAC:
                arr[arr <0] = 0
            data.append(arr)
